    ...
    for ent in prompt_ents:
        ...
        if f_name in wiki_fnames:
            ...
            if offline:  # in offline mode, we treat this as "evidence not found"
                ...
                wiki_url = "https://en.wikipedia.org/w/api.php?action=parse&page={}&prop=text&format=json".format(
                    wiki_doc_name
                )
                ...
                if (
                    len(wiki_obj.keys()) == 0
                ):  # IR fail case: wiki does not exist. empty page
                        ...
                        for t_obj in infobox_contents.find_all(["td", "th"]):
                            try:
                                ...
                                if "infobox-header" in class_name:
                                    ...
                                    infobox_dict[current_header].append(
                                        (current_label, current_data)
                                    )
                            except:
                                # print("ERROR:", t_obj)
                                continue
        ...
        for key in wiki_obj:
            if key == "Infobox":
                ...
                wiki_doc_sents = [
                    wiki_doc_name + " /// " + sent
                    for sent in sent_tokenize(wiki_content)
                ]
...
def obtain_relevant_evidences(claim, wiki_sents, k, method):
    ...
    if method == "tfidf" or method == "combined":
        ...
        relevance_scores = (
            cosine_similarity(wiki_candidate_vectors, query_vector)
            .reshape((-1,))
            .tolist()
        )
    ...
    if method == "emb_sim" or method == "combined":
        ...
        hits = util.semantic_search(
            q_embedding, ev_embeddings, top_k=k
        ...
        evs.extend(
            [
                (wiki_sents[int(id_dict["corpus_id"])], id_dict["score"])
                for id_dict in hits[0]
            ]
        )
