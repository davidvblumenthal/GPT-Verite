...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Supply path to tokenized dataset"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Dealing with slurm time limit. Save checkpoint just before time limit of job runs out"
    )
...
def main():
    ...
    # in the environment
    accelerator = (
        Accelerator(gradient_accumulation_steps=args.gradient_accumulation_steps, log_with=args.report_to, logging_dir=args.output_dir) if args.with_tracking else Accelerator()
    )
    ...
    elif args.tokenized_dataset is not None:
        print("Using tokenized dataset!")
        with accelerator.main_process_first():
            train_dataset = load_from_disk(args.tokenized_dataset)
            eval_dataset = load_from_disk(args.tokenized_dataset + "_eval")
            print(f"\nTokenized dataset found with values: {train_dataset}")
    ...
    if args.tokenized_dataset is None:
    ...
    if args.with_tracking:
        if accelerator.is_main_process:
            ...
            accelerator.init_trackers("Packing_Influence", experiment_config, init_kwargs={"wandb": {"allow_val_change": True, "group": "No_packing", "name": "Run_1", "id": "8umr188e", "resume": "must"}})
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if completed_steps % 100 == 0:
                    temp_loss = torch.mean(accelerate.utils.gather(batch_loss / accelerator.gradient_accumulation_steps / 100))
                    accelerator.print(f"current loss: {temp_loss}")
                    
                    accelerator.log(
                        {
                            "training_loss": torch.mean(accelerate.utils.gather(batch_loss / accelerator.gradient_accumulation_steps / 100)).item()
                        }, 
            ...
            # New Code added by David
            if (int(time()) - START_TIME) / 60 >= args.exit_duration:
                accelerator.print("Exiting and saving checkpoint due to exit_duration")
            ...
            if isinstance(checkpointing_steps, int):
                if completed_steps % checkpointing_steps == 0 and completed_steps > 1:
                    output_dir = f"step_{completed_steps }"
                    if args.output_dir is not None:
                        output_dir = os.path.join(args.output_dir, output_dir)
                    accelerator.save_state(output_dir)
